...
Scraper.prototype.start = function() {
    ...
    setInterval(this.scrapeCategory.bind(this), 15*60*1000);
    this.scrapeCategory();
    
    // scrape a new notice every minute
    //setInterval(this.scrapeNotice.bind(this), 60*1000);
};
...
Scraper.prototype.scrapeCategory = function() {
    ...
    Category.findOne({
        ...
        sort: {update: 1}
    }, scrapeCategory.bind(this));
};

function scrapeCategory(err, category) {
    category.scraping = true;
    category.save();
    new CategoryJob(scraper.db, category.slug, function(err) {
...
Scraper.prototype.end = function(err) {
    ...
    this.notifier.poll(this.startTime);
    if(err) {
