...
function scrapeNotice(notice, cat, cb) {
    ...
    Notice.findOne({
        ...
        if(err) {
            ...
            console.log('Create new notice:', notice.itemId);
            Notice.create(notice, cb);
        }
        else {
            if(noticeObj.Status != notice.Status) {
                console.log('Update notice:', notice.itemId);
                noticeObj.Status = notice.Status;
                noticeObj.save(function(err) {
                    console.log('notice updated', err);
                    cb(err);
                });
            }
            else {
                console.log('Notice is up to date:', notice.itemId);
                cb(null);
            }
        }
    });
